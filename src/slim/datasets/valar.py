...
_ITEMS_TO_DESCRIPTIONS = {
    ...
    'label': 'a list of two floats: [valence, arousal]',
    'text' : 'String describing the label'
}
...
def get_split(split_name, dataset_dir, file_pattern=None, reader=None):
  ...
  keys_to_features = {
      ...
      'image/colorspace': tf.FixedLenFeature((), tf.string, default_value=None),
      'image/channels': tf.FixedLenFeature((), tf.int64, default_value=3),
      'image/label': tf.FixedLenFeature([_NUM_CLASSES], tf.float32, 
          default_value=tf.zeros(_NUM_CLASSES, dtype=tf.float32)),
      'image/text': tf.FixedLenFeature((), tf.string, default_value=''),
